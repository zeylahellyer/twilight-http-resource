...
#[derive(Clone, Debug)]
pub struct WebhookMessageResource<'a>(&'a Client, WebhookId, &'a str);

impl<'a> WebhookMessageResource<'a> {
    /// Create a resource instance to work with a webhook's messages.
    #[must_use = "this is a builder and does nothing on its own"]
    pub const fn new(client: &'a Client, webhook_id: WebhookId, token: &'a str) -> Self {
        Self(client, webhook_id, token)
    }

    /// Delete a webhook message.
    #[must_use = "this is a builder and does nothing on its own"]
    pub const fn delete(self, message_id: MessageId) -> DeleteWebhookMessage<'a> {
        self.0.delete_webhook_message(self.1, self.2, message_id)
    }

    /// Update a webhook message.
    #[must_use = "this is a builder and does nothing on its own"]
    pub const fn patch(self, message_id: MessageId) -> UpdateWebhookMessage<'a> {
        self.0.update_webhook_message(self.1, self.2, message_id)
    }

    /// Create a webhook message.
    #[must_use = "this is a builder and does nothing on its own"]
    pub const fn post(self, id: WebhookId) -> ExecuteWebhook<'a> {
